...
function render() {
  ...
  document.getElementById('displayBio').textContent = profile.about || 'Tell us about yourself...';

  const avatarEl = document.getElementById('avatarDisplay');
  if (profile.profilepicURL) {
    avatarEl.innerHTML = '<img src="' + profile.profilepicURL + '" alt="Avatar">';
  } else {
    avatarEl.innerHTML = '<span class="avatarInitials" id="avatarInitials">' + (profile.firstName[0] || '') + (profile.lastName ? profile.lastName[0] : '') + '</span>';
  }
  ...
  if (profile.experiences.length === 0) {
    ...
    expEl.innerHTML = profile.experiences.map((exp, i) =>
      `<div class="entry">
  ...
  if (profile.educations.length === 0) {
    ...
    eduEl.innerHTML = profile.educations.map((edu, i) =>
      `<div class="entry">
        <div class="entry-left">
...
function hideAllForms() {
  document.querySelectorAll('.modalForm').forEach(el => el.style.display = 'none');
}
...
function validateFormFields(formId) {
  ...
  for (let input of inputs) {
    if (input.hasAttribute('required') || input.id.match(/firstName|lastName|mHeadline|mLocation|mEmail|jobTitle|company|workLocation|school|degree|fieldOfStudy|skillName/)) {
      if (!input.value.trim()) {
        return false;
      }
    }
...
function openEditProfile() {
  ...
  openModal('Edit Profile', 'Update your basic information.', 'formEditProfile', () => {
...
function openEditBio() {
  ...
  openModal('Edit About', 'Write a short summary about yourself.', 'formEditBio', () => {
...
function openAddExp() {
  clearForm('formExp');
  expModal({ id: uid(), title: '', company: '', location: '', startDate: '', endDate: '', description: '' }, false);
}
...
function openAddEdu() {
  clearForm('formEdu');
  eduModal({ id: uid(), school: '', degree: '', field: '', startYear: '', endYear: '' }, false);
}
...
function openAddSkill() {
  ...
  openModal('Add Skill', 'Add a new skill to your profile.', 'formSkill', () => {
